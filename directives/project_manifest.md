...
>
> **Next up (Sean's call, 2026-07-03): AoW is last in the queue.** Follow CT's/T5's/RTT's 5-item sequence (field-level/structural gating → write-stub gaps → UWP-auditor `sys.auditResult` coverage → `_ENGINE_ADAPTERS` entry → UI exposure). Per-engine remaining-work is in Section 5 below:
> - **AoW** — already the furthest *along* at the generator level (structural + field-level `seedSys` gating both present in `aow_bottomup_generator.js`), but has no adapter or UI exposure yet and hasn't had a UWP-auditor pass. It has no dedicated stub-inventory subsection in this manifest yet either (see the note below) — write that up first, per the Zero-Assumption Policy, before starting its Phase B implementation.
...
**Status as of 2026-07-04:** CT, T5, and RTT are all fully online. AoW is the only engine left in the Phase B queue.

The engine selection dialog HTML (`#se-engine-dialog`, in `hex_map.html`) has MgT2E / CT / T5 / AoW / RTT radio buttons present. **UPDATED 2026-07-04:** CT's, T5's, and RTT's are now all enabled (no longer `disabled`, "(coming soon)" label removed), matching MgT2E's. Only AoW remains `disabled` with the "(coming soon)" label. Remaining work here is *enabling* AoW's radio once its gate/audit/adapter work is done.
